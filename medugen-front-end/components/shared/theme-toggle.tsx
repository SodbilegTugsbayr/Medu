...
export default function ThemeToggle({ className = "" }: ThemeToggleProps) {
  ...
  if (!mounted) {
    return (
      <div
        className={`relative w-[60px] h-[30px] rounded-[50px] ${className}`}
      />
    );
  }
  ...
  return (
    <div
      className={`relative z-10 flex w-[150px] h-[60px] justify-center items-center perspective-[800px] ${className}`}
    >
      <div
        className="relative h-[60px] w-full [transform-style:preserve-3d]"
        style={{ transform: "scale(0.4)", transformOrigin: "center center" }}
      >
        <input
          id={id}
          type="checkbox"
          className="peer absolute h-0 w-0 opacity-0 toggle-input"
          checked={isDark}
      ...
      <style jsx>{`
        ...
        .toggle-track {
          ...
          border-radius: 30px;
          cursor: pointer;
          box-shadow: 0 0 15px rgba(0, 80, 255, 0.2),
